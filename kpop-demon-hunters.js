...
async function migrateLocalScoresToFirebase() {
  console.log('🔄 Checking for local K-pop scores to migrate...');
  
  const localScores = JSON.parse(localStorage.getItem('kpop-demon-huntersScores') || '[]');
  
  if (localScores.length === 0) {
    console.log('No local K-pop scores to migrate');
    return;
  }
  ...
  try {
    console.log(`🚀 Migrating ${localScores.length} local K-pop scores to Firebase...`);
    ...
    console.log('✅ Successfully migrated all K-pop scores to Firebase!');
    
    // Clear local scores after successful migration
    localStorage.removeItem('kpop-demon-huntersScores');
    console.log('🧹 Local K-pop scores cleared after migration');
...
async function checkFirebaseStatus() {
  console.log('🔍 Checking K-pop Firebase connection...');
  ...
  try {
    if (window.firebaseDB) {
      ...
      console.log('✅ K-pop Firebase connection successful!');
      console.log(`📊 Current K-pop scores in Firebase: ${snapshot.docs.length > 0 ? 'Found data' : 'Empty collection'}`);
      
      return true;
    } else {
      console.log('❌ Firebase not initialized for K-pop game');
      return false;
    }
  } catch (error) {
    console.log('❌ K-pop Firebase connection failed:', error.message);
    return false;
  }
}
...
async function saveScore() {
  ...
  try {
    // Save to Firebase with duplicate checking
    if (window.firebaseDB) {
      ...
      if (shouldUpdate && existingDocId) {
        ...
        await window.firebaseUpdateDoc(window.firebaseDoc(window.firebaseDB, 'kpop-demon-huntersScores', existingDocId), newScore);
      ...
      localStorage.removeItem('kpop-demon-huntersScores');
      console.log('Local scores cleared after successful Firebase operation');
    } else {
...
async function showLeaderboard() {
  ...
  leaderboardList.innerHTML = '<div style="text-align: center; color: #666;">🔄 Loading K-pop leaderboard...</div>';
  ...
  try {
    ...
    if (window.firebaseDB) {
      console.log('🔥 Attempting to load K-pop scores from Firebase...');
      ...
      console.log(`✅ Successfully loaded ${scores.length} K-pop scores from Firebase`);
      ...
      if (firebaseHasScores) {
        ...
        const localScores = JSON.parse(localStorage.getItem('kpop-demon-huntersScores') || '[]');
        if (localScores.length > 0) {
          console.log('🧹 Clearing local K-pop scores - using Firebase data');
          localStorage.removeItem('kpop-demon-huntersScores');
        }
      }
    } else {
  ...
  if (!firebaseAvailable || !firebaseHasScores) {
    ...
    if (localScores.length > 0) {
      scores = localScores;
      dataSource = 'local';
      console.log(`💾 Using ${scores.length} local K-pop scores as fallback`);
    } else {
      scores = [];
      dataSource = 'empty';
      console.log('📭 No K-pop scores found anywhere');
    }
  }
